...
describe("stream-factory", () => {
  ...
  it("total-batch-calls increments after create-payroll-stream", () => {
    setupAll();
    const start = simnet.blockHeight + 2;
    simnet.callPublicFn("stream-factory", "create-payroll-stream",
      [Cl.principal(wallet2), Cl.uint(50_000), Cl.uint(3), Cl.uint(start)], wallet1);
  ...
  it("total-batch-calls accumulates across multiple batch operations", () => {
    setupAll();
    const start = simnet.blockHeight + 2;
    simnet.callPublicFn("stream-factory", "create-payroll-stream",
      [Cl.principal(wallet2), Cl.uint(50_000), Cl.uint(3), Cl.uint(start)], wallet1);
    simnet.callPublicFn("stream-factory", "create-vesting-stream",
      [Cl.principal(wallet2), Cl.uint(100_000), Cl.uint(10), Cl.uint(500)], wallet1);
    simnet.callPublicFn("stream-factory", "create-payroll-stream",
      [Cl.principal(wallet2), Cl.uint(30_000), Cl.uint(2), Cl.uint(start)], wallet1);
    const { result } = simnet.callReadOnlyFn("stream-factory", "get-total-batch-calls", [], deployer);
...
describe("stream-factory payroll stream structure", () => {
  ...
  it("create-payroll-stream total-amount equals monthly-amount times months", () => {
    simnet.callPublicFn("stream-vault", "set-registry",
      [Cl.principal(`${deployer}.stream-registry`)], deployer);
    const monthlyAmount = 30_000;
    const months = 3;
    const start = simnet.blockHeight + 2;
    simnet.callPublicFn("stream-factory", "create-payroll-stream",
      [Cl.principal(wallet2), Cl.uint(monthlyAmount), Cl.uint(months), Cl.uint(start)], wallet1);
    const { result } = simnet.callReadOnlyFn("stream-registry", "get-stream-remaining", [Cl.uint(0)], deployer);
    expect(result).toBeOk(Cl.uint(monthlyAmount * months));
  });

  it("create-vesting-stream duration equals the vesting-blocks argument", () => {
    simnet.callPublicFn("stream-vault", "set-registry",
      [Cl.principal(`${deployer}.stream-registry`)], deployer);
    const vestingBlocks = 500;
    simnet.callPublicFn("stream-factory", "create-vesting-stream",
      [Cl.principal(wallet2), Cl.uint(100_000), Cl.uint(10), Cl.uint(vestingBlocks)], wallet1);
    const { result } = simnet.callReadOnlyFn("stream-registry", "get-stream-duration", [Cl.uint(0)], deployer);
    expect(result).toBeOk(Cl.uint(vestingBlocks));
  });
